...
serve(async (req) => {
  ...
  try {
    ...
    if (!plan.price || plan.price <= 0) return new Response(JSON.stringify({ error: 'Free plan cannot be purchased' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })

    const razorpay = new Razorpay({ key_id: Deno.env.get('RAZORPAY_KEY_ID')!, key_secret: Deno.env.get('RAZORPAY_KEY_SECRET')! })
    const order = await razorpay.orders.create({ amount: plan.price, currency: 'INR', receipt: `yt_${user.id}_${Date.now()}` })

    return new Response(JSON.stringify({
      id: order.id, amount: order.amount, currency: order.currency,
      key_id: Deno.env.get('RAZORPAY_KEY_ID'), plan_name: plan.name,
      plan_id: plan.id, user_email: user.email, user_name: user.email
